...
import { decodeSession, findBrokenToolCalls } from './session-log.js';
/**
 ...
 */
export function checkProfileShadowing(profileDir) {
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 */
export function checkManifestBom(profileDir) {
 ...
 */
export function checkLargeFiles(profileDir, thresholdBytes = 100 * 1024 * 1024) {
    ...
    const walk = (dir, depth) => {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                if (stat.isDirectory())
                    walk(full, depth + 1);
                else if (stat.size >= thresholdBytes)
                    large.push({ rel: path.relative(profileDir, full), size: stat.size });
            }
            catch { /* unreadable */ }
        }
    };
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 */
export function checkEntryPoints(profileDir) {
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 */
export function checkProfileDeps(profileDir) {
    ...
    if (found !== undefined) {
        let count = 0;
        try {
            count = readdirSync(path.join(found, '@deepseek-ai')).length;
        }
        catch { /* unreadable */ }
 ...
 * @param profileDir - absolute path of the dsh profile to inspect.
 */
export function checkNativeModules(profileDir) {
    const candidates = [path.join(profileDir, 'node_modules'), path.resolve(profileDir, '..', 'node_modules')];
    const present = [];
    const missing = [];
    for (const pkg of ['koffi', 'node-pty']) {
        if (candidates.some(dir => existsSync(path.join(dir, pkg))))
            present.push(pkg);
        else
            missing.push(pkg);
    }
 ...
 */
export function checkToolCallPairing(dir) {
    ...
    const walk = (d, depth) => {
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                if (s.isDirectory())
                    walk(full, depth + 1);
                else if (entry === 'session.jsonl' || entry === 'session.jsonl.zstd')
...
function collectSourceFiles(dir, out = [], depth = 0) {
    ...
    for (const entry of entries) {
        if (entry === 'node_modules' || entry === '.git' || entry === '.DS_Store'
            || entry === 'tests' || entry === 'test' || entry === '__tests__')
            continue;
        const full = path.join(dir, entry);
        try {
            const stat = statSync(full);
 ...
 */
export function checkPreExecuteSideEffects(dir) {
    const root = path.resolve(dir);
 ...
 */
export function checkShellLauncher(dir) {
    const root = path.resolve(dir);
    const files = collectSourceFiles(root);
    const hits = [];
    for (const file of files) {
 ...
 */
export function checkSupplyChainSecurity(dir) {
    const root = path.resolve(dir);
...
export function readManifest(dir) {
    ...
    try {
        ...
        const dsh = raw.dsh;
        const bundle = dsh !== null && typeof dsh === 'object' ? dsh.bundle : null;
        return {
            name: typeof raw.name === 'string' ? raw.name : undefined,
            version: typeof raw.version === 'string' ? raw.version : undefined,
            main: typeof raw.main === 'string' ? raw.main : undefined,
            files: Array.isArray(raw.files) ? raw.files.filter((x) => typeof x === 'string') : [],
            prepare: typeof raw.scripts === 'object' && raw.scripts !== null
                ? raw.scripts.prepare
                : undefined,
            patch: bundle !== null && typeof bundle === 'object' ? bundle.patch : undefined,
...
export function parsePatch(content) {
    ...
    if (!Array.isArray(parsed))
        throw new Error('patch file must be a YAML list of operations');
    const ids = [];
 ...
 */
export async function doctor(dir, options = {}) {
    ...
    if (!manifest.dshBundle)
        manifestProblems.push('missing dsh.bundle manifest');
    if (manifest.patch === undefined)
        manifestProblems.push('missing dsh.bundle.patch');
    if (manifest.prepare === undefined)
        manifestProblems.push('missing prepare script (required for git installs)');
    if (manifest.main === undefined)
        manifestProblems.push('missing main entry');
    checks.push({
        name: 'manifest',
