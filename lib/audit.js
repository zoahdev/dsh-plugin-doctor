...
function evidenceAt(relativePath, text, index) {
...
function evidenceForNeedle(relativePath, text, needle) {
    const index = text.indexOf(needle);
    return evidenceAt(relativePath, text, index === -1 ? 0 : index);
}
function ignoredDynamicCodeRanges(text) {
    const ranges = [];
    let state = 'code';
    let start = 0;
    let escaped = false;
...
function findEvidence(relativePath, text, pattern, ignoredRanges = []) {
...
function fingerprint(ruleId, evidence) {
...
function finding(ruleId, category, severity, confidence, title, explanation, evidence, recommendation) {
    return {
...
function addSkipped(coverage, relativePath, reason) {
    coverage.skippedFiles += 1;
    coverage.skippedByReason[reason] = (coverage.skippedByReason[reason] ?? 0) + 1;
    if (coverage.skippedExamples.length < 40)
        coverage.skippedExamples.push({ path: relativePath, reason });
}
function discoverTextFiles(root, coverage, scanRoots) {
    const files = new Map();
    const seen = new Set();
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (coverage.scanMode === 'working-tree' && WORKING_TREE_NON_RUNTIME_DIRECTORIES.has(entry.name)) {
                    ...
                    if (coverage.skippedExamples.length < 40)
                        coverage.skippedExamples.push({ path: relativePath, reason: 'working-tree-non-runtime' });
                    continue;
                }
...
function publishedScanRoots(root, manifest) {
    ...
    if (patchFile !== undefined) {
        ...
        if (resolvedPatch === null)
            unsafePaths.push(patchFile);
        else
            roots.push(resolvedPatch);
    }
    const mainFile = stringValue(manifest.main);
    if (mainFile !== undefined) {
        const resolvedMain = resolveInsideRoot(root, mainFile);
        if (resolvedMain === null)
            unsafePaths.push(mainFile);
        else if (!existsSync(resolvedMain))
            return { mode: 'working-tree', paths: declared, roots: Array.from(new Set([root, ...roots])), unsafePaths };
    }
    const hasExistingPublishedPath = roots.some(item => item !== path.join(root, 'package.json') && existsSync(item));
    if (!hasExistingPublishedPath)
        return { mode: 'working-tree', paths: declared, roots: Array.from(new Set([root, ...roots])), unsafePaths };
    return { mode: 'published-files', paths: declared, roots, unsafePaths };
}
function readLocalGit(root) {
...
function manifestScripts(manifest) {
    if (manifest === null || !isRecord(manifest.scripts))
        return {};
    return Object.fromEntries(Object.entries(manifest.scripts).filter((entry) => typeof entry[1] === 'string'));
}
function dependencySourceType(spec) {
    if (/^(?:git\+|git:|github:|gitlab:|bitbucket:)/i.test(spec) || /^[\w.-]+\/[\w.-]+(?:#.*)?$/.test(spec))
        return 'git';
    ...
    if (/^workspace:/i.test(spec))
        return 'workspace';
    if (/^(?:latest|next|beta|canary|dev)$/i.test(spec) || spec === '*')
        return 'tag';
...
function packageJsonEvidence(text, key) {
    return evidenceForNeedle('package.json', text, `"${key}"`);
}
function inspectDependencies(root, manifest, manifestText, coverage, findings, lifecycleScripts) {
    const observations = [];
    for (const section of ['dependencies', 'optionalDependencies', 'peerDependencies']) {
        ...
        for (const [name, rawSpec] of Object.entries(values)) {
            ...
            if (section !== 'peerDependencies')
                coverage.declaredRuntimeDependencies += 1;
            ...
            if (!validDependencyName) {
                findings.push(finding('dependency-name-invalid', 'dependency', 'high', 'high', `Dependency name ${name} is not a valid npm package name`, 'Invalid dependency keys can be used to confuse local path inspection and do not identify a normal registry package.', [packageJsonEvidence(manifestText, name)], 'Use a valid npm package name and keep local sources in the dependency value, not the key.'));
            }
            ...
            if (sourceType === 'git' || sourceType === 'url') {
                ...
                findings.push(finding(`dependency-${sourceType}-source`, 'dependency', 'medium', 'high', `Dependency ${name} is installed from a ${sourceType === 'git' ? 'Git repository' : 'remote URL'}`, 'Remote and Git dependency specifications are harder to reproduce and may change outside normal registry integrity checks.', [evidence], 'Pin an immutable commit or registry version and record its integrity metadata.'));
            }
            else if (sourceType === 'file') {
                const evidence = packageJsonEvidence(manifestText, name);
                findings.push(finding('dependency-local-source', 'dependency', 'low', 'high', `Dependency ${name} uses a local file or link`, 'Local dependency sources are not reproducible on another machine and can hide code outside the reviewed package directory.', [evidence], 'Publish or vendor the exact dependency content and record a digest.'));
            }
            else if (sourceType === 'tag') {
                const evidence = packageJsonEvidence(manifestText, name);
                findings.push(finding('dependency-floating-tag', 'dependency', 'low', 'high', `Dependency ${name} uses a floating tag`, 'Tags such as latest or next can resolve to different code at different installation times.', [evidence], 'Use a bounded version range and commit a lockfile.'));
            }
        }
    }
...
function parseConfigurationChanges(patchPath, root, findings) {
    ...
    if (!Array.isArray(parsed))
        return [];
    const sandboxReplacementEvidence = [];
    for (const operation of parsed) {
        if (!isRecord(operation) || !Array.isArray(operation.insert))
            continue;
        for (const row of operation.insert) {
            ...
            if (!/sandbox/i.test(`${entryId ?? ''} ${moduleName ?? ''}`))
                continue;
            sandboxReplacementEvidence.push(evidenceForNeedle(relativePath, text, entryId === undefined ? '- insert:' : `id: ${entryId}`));
        }
    }
    for (const operation of parsed) {
        ...
        if (entryId !== undefined) {
            ...
            if (/danger-full-access/i.test(serialized)) {
                ...
                findings.push(finding('config-danger-full-access', 'configuration', 'high', 'high', conditional
                    ? `Configuration ${entryId} can enable unrestricted host access`
                    : `Configuration ${entryId} enables unrestricted host access`, conditional
                    ...
                    : 'The patch contains danger-full-access, which removes normal workspace restrictions for the affected execution path.', dangerEvidence.length > 0 ? dangerEvidence : [change.evidence], 'Require an explicit user decision and document why narrower access is insufficient.'));
            }
            if (/"policy":"never"/i.test(serialized) || (/approval/i.test(entryId) && operation.disabled === true)) {
                findings.push(finding('config-approval-disabled', 'configuration', 'high', 'high', `Configuration ${entryId} removes an approval check`, 'The patch sets approval policy to never or disables an approval-related entry.', [change.evidence], 'Keep approval enabled by default and make any bypass an explicit installation-time choice.'));
            }
            if (/sandbox/i.test(entryId) && operation.disabled === true) {
                const hasReplacement = sandboxReplacementEvidence.length > 0;
                findings.push(finding('config-sandbox-disabled', 'configuration', hasReplacement ? 'medium' : 'high', 'high', hasReplacement
                    ? `Configuration ${entryId} replaces a sandbox entry`
                    : `Configuration ${entryId} disables a sandbox entry`, hasReplacement
                    ...
                    : 'Disabling a sandbox entry can move commands or file operations onto the unrestricted host.', hasReplacement ? [change.evidence, ...sandboxReplacementEvidence] : [change.evidence], hasReplacement
    ...
    if (expressions.length > 0) {
        findings.push(finding('config-runtime-expression', 'configuration', 'info', 'high', 'Configuration contains runtime JavaScript expressions', 'Runtime expressions make the effective configuration depend on the host environment and should be included in manual review.', expressions));
    }
    return changes;
}
...
function classifyNetworkLiteral(literal) {
    if (/^(?:\/|\.\.?\/)/.test(literal))
        return 'same-origin';
    if (!/^(?:https?|wss?):\/\//i.test(literal))
        return 'unknown';
    if (/^(?:https?|wss?):\/\/(?:localhost|127(?:\.\d{1,3}){3}|\[?::1\]?)(?=[:/]|$)/i.test(literal))
        return 'same-origin';
    try {
...
function networkDestinationSymbols(text) {
    ...
    while ((assignment = literalAssignments.exec(text)) !== null) {
        ...
        if (destination !== 'unknown')
            symbols.set(assignment[1] ?? '', destination);
    }
    const sameOriginUrls = /\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*new\s+URL\s*\([^,]+,\s*(?:(?:globalThis|window|document)\.)?location\.origin\s*\)/g;
    while ((assignment = sameOriginUrls.exec(text)) !== null)
        symbols.set(assignment[1] ?? '', 'same-origin');
    return symbols;
}
function classifyNetworkArgument(fragment, symbols) {
    const argument = fragment.trimStart();
    if (/^(?:(?:globalThis|window|document)\.)?location\.origin\b/.test(argument))
        return 'same-origin';
    if (/^new\s+URL\s*\([^,]+,\s*(?:(?:globalThis|window|document)\.)?location\.origin\b/.test(argument))
        return 'same-origin';
    ...
    if (literal !== undefined) {
        ...
        if (interpolatedSymbol !== undefined)
            return symbols.get(interpolatedSymbol) ?? 'unknown';
    }
    const symbol = /^([A-Za-z_$][\w$]*)(?:\.toString\s*\(\s*\))?/.exec(argument)?.[1];
    if (symbol !== undefined)
        return symbols.get(symbol) ?? 'unknown';
    return 'unknown';
}
function inspectNetworkDestinations(file) {
    const result = { external: [], sameOrigin: [], unknown: [] };
    ...
    while ((match = matcher.exec(file.text)) !== null) {
        ...
        if (destination === 'same-origin')
            result.sameOrigin.push(evidence);
        else
            result[destination].push(evidence);
        if (match[0].length === 0)
            matcher.lastIndex += 1;
    }
    const deduplicate = (items) => Array.from(new Map(items.map(item => [`${item.path}:${item.line}:${item.excerpt}`, item])).values()).slice(0, MAX_EVIDENCE_PER_RULE);
    return {
...
function isAncillaryFile(file) {
    const normalized = file.relativePath.replace(/\\/g, '/');
    const segments = normalized.split('/').map(segment => segment.toLowerCase());
    if (segments.some(segment => ['scripts', 'script', 'evals', 'eval', 'recipes', 'recipe', 'benchmarks', 'benchmark', 'examples', 'example', 'demos', 'demo', 'vendor', 'third-party', 'third_party'].includes(segment)))
        return true;
    const basename = path.posix.basename(normalized).toLowerCase();
    return /(?:^|[-_.])(?:verify|smoke|benchmark|bench|fixture|example|demo|test|spec)(?:[-_.]|$)/.test(basename);
}
function inspectCapabilities(files, findings) {
    const capabilities = [];
    ...
    const evidenceByFileAndKind = new Map();
    for (const rule of CAPABILITY_RULES) {
        const evidence = [];
        for (const file of capabilityFiles) {
            if (file.relativePath === 'package.json' || file.relativePath.endsWith('.yaml') || file.relativePath.endsWith('.yml'))
                continue;
            const matches = findEvidence(file.relativePath, file.text, rule.pattern, rule.kind === 'dynamic-code' ? dynamicIgnoredByFile.get(file.relativePath) ?? [] : []);
            if (matches.length === 0)
                continue;
            evidence.push(...matches);
            const kinds = byFile.get(file.relativePath) ?? new Set();
            kinds.add(rule.kind);
    ...
    for (const capability of capabilities) {
        if (capability.kind === 'dynamic-code') {
            ...
            findings.push(finding('code-dynamic-execution', 'code', weakRuntimeEvidence ? 'medium' : 'high', weakRuntimeEvidence ? 'low' : 'high', onlyAncillaryEvidence
                ? 'An ancillary published tool contains dynamic code execution'
                : onlyBundledEvidence
                    ? 'A generated bundle contains dynamic code execution'
                    : 'Plugin can execute dynamically constructed code', onlyAncillaryEvidence
                ...
                : onlyBundledEvidence
                    ...
                    : 'Static inspection cannot determine what eval, Function, or vm execution will run.', capability.evidence, 'Remove dynamic execution or require focused manual review of every input reaching it.'));
        }
            ...
            findings.push(finding('host-persistence-surface', 'code', weakRuntimeEvidence ? 'medium' : 'high', weakRuntimeEvidence ? 'low' : 'medium', weakRuntimeEvidence ? 'A generated or ancillary file references host persistence mechanisms' : 'Plugin references host persistence mechanisms', weakRuntimeEvidence
                ? 'The match is not in a clearly attributable plugin runtime path and requires lower-confidence review.'
                : 'Startup tasks, shell profiles, service managers, and authorized_keys can make code survive beyond the current DSH process.', capability.evidence, 'Confirm the behavior is essential, disclosed, reversible, and explicitly approved.'));
        }
        else if (capability.kind === 'native-code') {
            findings.push(finding('native-code-surface', 'code', 'medium', 'medium', 'Plugin may load or build native code', 'Native modules add platform-specific binaries and code outside ordinary JavaScript inspection.', capability.evidence, 'Record supported binaries, build provenance, checksums, and install-script behavior.'));
        }
    }
    for (const [relativePath, kinds] of byFile) {
        ...
        const combinationConfidence = likelyBundled || ancillary ? 'low' : 'medium';
        const networkDestinations = networkByFile.get(relativePath) ?? { external: [], sameOrigin: [], unknown: [] };
        const networkDestination = networkDestinations.external.length > 0
            ? 'external'
        ...
        const outboundNetworkEvidence = networkDestination === 'external'
            ...
            : networkDestination === 'unknown'
                ...
                : [];
        const combineEvidence = (...requested) => requested.flatMap(kind => kind === 'network' ? outboundNetworkEvidence : evidenceByFileAndKind.get(`${relativePath}\0${kind}`) ?? []).slice(0, MAX_EVIDENCE_PER_RULE);
        const dataCombination = (kind) => {
            const sensitive = evidenceByFileAndKind.get(`${relativePath}\0${kind}`) ?? [];
            const minimumDistance = sensitive.reduce((minimum, left) => Math.min(minimum, ...outboundNetworkEvidence.map(right => Math.abs(left.line - right.line))), Number.POSITIVE_INFINITY);
        ...
        if (kinds.has('credentials') && networkDestination !== null) {
            ...
            const baseSeverity = external ? (likelyBundled ? 'medium' : 'high') : (likelyBundled ? 'low' : 'medium');
            findings.push(finding('credential-and-network-combination', 'data-access', ancillary ? lowerSeverity(lowerSeverity(baseSeverity)) : combination.distant ? lowerSeverity(baseSeverity) : baseSeverity, combination.confidence, external
                ? 'Credential access and external network access occur in the same file'
                : 'Credential access and a dynamic network destination occur in the same file', ancillary
                ...
                : combination.distant
                    ...
                    : external
                        ...
                        : 'The destination could not be determined statically. This is a review signal, not proof that credentials leave the machine.', combination.evidence, 'Trace the data flow manually and restrict destinations and credential names.'));
        }
        if (kinds.has('session-data') && networkDestination !== null) {
            const combination = dataCombination('session-data');
            const external = networkDestination === 'external';
            const baseSeverity = external ? (likelyBundled ? 'medium' : 'high') : (likelyBundled ? 'low' : 'medium');
            findings.push(finding('session-data-and-network-combination', 'data-access', ancillary ? lowerSeverity(lowerSeverity(baseSeverity)) : combination.distant ? lowerSeverity(baseSeverity) : baseSeverity, combination.confidence, external
                ? 'Session data access and external network access occur in the same file'
                : 'Session data access and a dynamic network destination occur in the same file', ancillary
                ? 'The two capabilities occur in an ancillary published script, evaluation, recipe, example, or verification tool rather than an identified plugin runtime path.'
                : combination.distant
                    ? 'The two capabilities occur in distant sections of the file, so co-location is weak evidence and does not establish a data flow.'
                    : external
                        ? 'The file has the surface required to transmit DSH conversation or storage data to an external destination.'
                        : 'The destination could not be determined statically. The file needs review, but this does not prove session data leaves the machine.', combination.evidence, 'Verify the exact transmitted fields, destination allowlist, consent, and retention policy.'));
        }
        if (networkDestination !== null && kinds.has('dynamic-code')) {
            const evidence = combineEvidence('network', 'dynamic-code');
            const baseSeverity = networkDestination === 'external'
                ? (likelyBundled ? 'medium' : 'high')
                : (likelyBundled ? 'low' : 'medium');
            findings.push(finding('download-and-dynamic-code-surface', 'code', ancillary ? lowerSeverity(lowerSeverity(baseSeverity)) : baseSeverity, combinationConfidence, 'Outbound network access and dynamic code execution occur in the same file', 'The file has the surface required to retrieve content and execute it through eval, Function, or vm. Static inspection does not prove that the two paths are connected.', evidence, 'Verify data flow, pin downloaded content by digest, and avoid executing mutable remote content.'));
        }
        else if (networkDestination !== null && kinds.has('process')) {
            const evidence = combineEvidence('network', 'process');
            const baseSeverity = networkDestination === 'external' ? 'medium' : 'low';
            findings.push(finding('network-and-process-surface', 'code', ancillary ? lowerSeverity(baseSeverity) : baseSeverity, combinationConfidence, 'Outbound network access and process execution occur in the same file', 'The file can communicate over the network and start local processes. Static inspection does not prove that downloaded data reaches a command.', evidence, 'Review the data flow, executable allowlist, arguments, and network destinations.'));
        }
    }
    return capabilities.sort((left, right) => left.kind.localeCompare(right.kind));
}
function inspectOwnLifecycleScripts(manifest, manifestText, findings) {
    const observations = [];
    ...
    for (const [script, command] of Object.entries(manifestScripts(manifest))) {
        ...
        findings.push(finding(suspicious ? 'install-script-network-or-shell' : 'install-lifecycle-script', 'install', suspicious ? 'high' : 'medium', 'high', suspicious ? `Lifecycle script ${script} uses a network or shell surface` : `Lifecycle script ${script} runs during installation or publication`, suspicious
            ? 'The package can run a shell or retrieve remote content before the plugin audit or runtime protections begin.'
            : 'Lifecycle scripts execute outside the DSH runtime and therefore bypass runtime plugin checks.', [evidence], suspicious
...
function summarize(findings) {
    ...
    for (const item of findings) {
        ...
        if (SEVERITY_ORDER[item.severity] > SEVERITY_ORDER[highestSeverity])
            highestSeverity = item.severity;
    }
    const exitCode = bySeverity.critical + bySeverity.high > 0
        ? 2
        : bySeverity.medium > 0
            ? 1
            : 0;
...
export function auditPlugin(directory) {
    ...
    if (scanScope.unsafePaths.length > 0) {
        ...
        findings.push(finding('manifest-path-escape', 'provenance', 'high', 'high', 'Manifest paths escape the plugin directory', 'The scanner refused package files, main entries, or bundle patches that resolve outside the inspected plugin root.', evidence, 'Keep every published file, main entry, and DSH patch inside the package directory.'));
    }
    if (!existsSync(root)) {
        findings.push(finding('subject-missing', 'provenance', 'critical', 'high', 'Plugin directory does not exist', 'No plugin contents could be inspected.', [{ path: '.', line: 1, excerpt: root }]));
    }
    else if (manifest === null) {
        findings.push(finding('manifest-missing-or-invalid', 'provenance', 'high', 'high', 'package.json is missing or invalid', 'Package identity, scripts, dependencies, and DSH bundle metadata cannot be verified.', [{ path: 'package.json', line: 1, excerpt: manifestText === '' ? '(missing)' : redactExcerpt(manifestText.split('\n')[0] ?? '') }]));
    }
    ...
    for (const observation of lifecycleScripts.filter(item => !item.direct)) {
        findings.push(finding('dependency-lifecycle-script', 'dependency', 'medium', 'high', `Installed dependency ${observation.packageName} has a ${observation.script} script`, 'Dependency lifecycle scripts can execute during installation before DSH runtime checks begin.', [observation.evidence], 'Review the exact command and verify the installed package source and integrity.'));
    }
    ...
    if (patchRelative !== undefined) {
        ...
        if (patchPath !== null)
            configurationChanges = parseConfigurationChanges(patchPath, root, findings);
    }
    ...
    if (provenance.repository === undefined && provenance.localGitRemote === undefined) {
        findings.push(finding('provenance-source-missing', 'provenance', 'low', 'high', 'No source repository is declared', 'The local package does not declare a repository and is not a readable Git checkout, so reviewers cannot link the contents to a source history.', [packageJsonEvidence(manifestText, 'name')], 'Declare package.json repository metadata and publish from a traceable source revision.'));
    }
    if (provenance.registryIntegrity === undefined) {
        findings.push(finding('provenance-registry-integrity-unavailable', 'provenance', 'info', 'high', 'Registry integrity metadata is unavailable in this directory', 'The scanner computed a local content digest, but it cannot prove that these files match an npm registry tarball without registry or lockfile metadata.', [packageJsonEvidence(manifestText, 'version')]));
    }
...
function configurationKey(change) {
...
export function comparePluginAudits(baseline, current) {
...
export function auditPluginEcosystem(directories) {
...
export function aggregatePluginAudits(reports) {
    ...
    const byHighestSeverity = { info: 0, low: 0, medium: 0, high: 0, critical: 0 };
    ...
    for (const plugin of plugins) {
        ...
        for (const [ruleId, severity] of pluginRules) {
            ...
            current.count += 1;
            if (SEVERITY_ORDER[severity] > SEVERITY_ORDER[current.highestSeverity])
