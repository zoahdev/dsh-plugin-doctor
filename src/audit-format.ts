...
export function formatAuditReport(report: PluginAuditReport, comparison?: PluginAuditComparison): string {
  ...
  for (const item of report.findings) {
    ...
    for (const evidence of item.evidence) lines.push(`  - ${evidence.path}:${evidence.line} ${evidence.excerpt}`)
  }
...
export function formatEcosystemAuditReport(report: PluginEcosystemAuditReport): string {
  const lines = [
...
export function formatEcosystemAuditMarkdown(report: PluginEcosystemAuditReport): string {
  const lines = [
    ...
    '|---|---:|',
    ...(['critical', 'high', 'medium', 'low', 'info'] as const).map(severity => `| ${severity} | ${report.summary.byHighestSeverity[severity]} |`),
    '',
  ...
  lines.push(
    ...
    '',
    '- This is static inspection. A clean report is not proof that a plugin is safe.',
    '- Capability observations show what code can touch, not whether that access is malicious.',
    '- Large generated bundles lower the confidence of same-file capability combinations.',
    '- Registry signatures, npm provenance, vulnerability databases, ownership history, and runtime behavior require separate online or isolated checks.',
    '- Each embedded plugin report records its own skipped files and unresolved dependencies.',
    '',
  )
  return lines.join('\n')
}
